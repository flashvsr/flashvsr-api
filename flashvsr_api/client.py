"""FlashVSR API client — thin, dependency-light wrapper over the Synexa prediction API.
...
DEFAULT_MODEL = "bytedance/seedvr2-upscale"
MODELS = {
    "bytedance/seedvr2-upscale": {
        ...
        "url": "https://synexa.ai/explore/bytedance/seedvr2-upscale"
    }
}
...
class Client:
    def __init__(self, api_key: Optional[str] = None, base_url: str = API_BASE, timeout: float = 60.0):
        ...
        self._http = httpx.Client(timeout=timeout, headers={"x-api-key": self.api_key, "user-agent": "flashvsr_api/0.1.0"})
...
def run(input: Dict[str, Any], model: str = DEFAULT_MODEL, **kw):
    """Module-level shortcut: `flashvsr_api.run({...})` using SYNEXA_API_KEY from the environment."""
    global _default
    if _default is None:
        _default = Client()
